...
export async function POST(request) {
  ...
  const products = JSON.parse(await fs.readFile(productsFile, 'utf8')); const lines = input.items.map((item) => { const p = products.find((x) => x.id === item.productId || x.slug === item.productId || x.slug === item.slug); const qty = Math.max(1, Number(item.qty || 1)); if (!p || p.inventory < qty) throw new Error(`Unavailable product: ${item.productId || item.slug}`); return { productId: p.id, name: p.name, qty, unitPrice: p.price, amount: p.price * qty } })
