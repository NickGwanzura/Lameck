...
export default function Home() {
  ...
  return <><SiteHeader active="home" /><main><section className="hero" id="home"><div className="hero-image"/><div className="hero-copy container"><p className="kicker">The quiet art of living well</p><h1>Technology with<br/><em>presence.</em></h1><p>Refrigeration, entertainment, phones, and solar power chosen for the spaces you call home.</p><a className="button button-light" href="#shop">Shop the categories <span>↗</span></a></div></section>
    ...
    <section className="section section-stone" id="shop"><div className="container"><div className="section-title"><div><p className="kicker">Featured products</p><h2>New arrivals</h2></div><div className="filters">{['All', ...categories.map(([name]) => name)].map((name) => { const value = name === 'TV' ? 'tv' : name === 'All' ? 'All' : name.toLowerCase(); return <button key={name} className={`filter ${filter === value ? 'active' : ''}`} onClick={() => setFilter(value)}>{name}</button> })}</div></div><div className="product-grid">{visible.map((product, index) => <ProductCard product={product} badge={index % 2 === 0 ? 'New' : 'Featured'} key={product.id} />)}</div></div></section>
