...
export default function Home() {
  ...
  return <>
    ...
    <main><section className="hero" id="home"><div className="hero-image"/><div className="hero-copy container"><p className="kicker">The quiet art of living well</p><h1>Objects with<br/><em>presence.</em></h1><p>Enduring appliances and furniture, chosen for the spaces you call home.</p><a className="button button-light" href="#shop">Shop the collection <span>↗</span></a></div></section>
      <section className="service-strip casora-benefits"><div><span className="benefit-icon">✦</span><strong>Curated materials</strong><span>Built to be lived with</span></div><div><span className="benefit-icon">⌁</span><strong>White glove delivery</strong><span>From our door to yours</span></div><div><span className="benefit-icon">◇</span><strong>Two-year warranty</strong><span>Quality you can trust</span></div><div><span className="benefit-icon">↗</span><strong>Expert guidance</strong><span>Here when you need us</span></div></section>
      ...
      <section className="casora-highlight"><div className="highlight-image" /><div className="highlight-copy"><p className="kicker">The considered edit</p><h2>Designed to be lived with.</h2><p>From the first coffee to the last light, discover pieces that make everyday rituals feel intentional.</p><a className="button button-dark" href="/collections">Explore the edit&nbsp;↗</a></div></section>
      <section className="section section-stone" id="shop"><div className="container"><div className="section-title"><div><p className="kicker">Featured products</p><h2>New arrivals</h2></div><div className="filters">{['All','Appliances','Furniture'].map((f) => <button key={f} className={`filter ${filter === f ? 'active' : ''}`} onClick={() => setFilter(f)}>{f}</button>)}</div></div><div className="product-grid">{visible.map(([brand, name, price, image], index) => <article className="product" key={name}><div className={`product-image ${image}`}><span className="product-badge">{index % 2 === 0 ? 'New' : 'Featured'}</span><button className="wishlist" aria-label={`Add ${name} to wishlist`}>♡</button></div><p className="product-brand">{brand}</p><h3>{name}</h3><div className="product-card-row"><p className="price">{price}</p><button className="add-btn" onClick={() => addItem({ slug: name.toLowerCase().replace(/[’']/g, '').replace(/[^a-z0-9]+/g, '-').replace(/-$/, ''), name, brand, price, image })}>Quick add&nbsp;↗</button></div></article>)}</div></div></section>
